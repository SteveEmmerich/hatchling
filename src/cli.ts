...
const main = defineCommand({
  ...
  subCommands: {
    ...
    reflection: defineCommand({
      meta: {
        description: "Inspect reflection and behavior tuning signals",
      },
      args: {
        json: {
          type: "boolean",
      ...
      async run({ args }) {
        ...
        const rootDir = getInstancePath(activeInstance);
        const { getReflectionStatus, formatReflectionStatus } = await import("./brain/reflection_status.js");
        const status = await getReflectionStatus(rootDir);
        if (args.json) {
          console.log(JSON.stringify(status, null, 2));
          return;
        }
        console.log(formatReflectionStatus(status));
      },
    }),
