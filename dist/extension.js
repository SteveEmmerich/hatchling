...
export default function (pi) {
    ...
    pi.on("session_start", async (_event, ctx) => {
        ...
        try {
            ...
            if (model) {
                ...
                if (!ok) {
                    ctx.ui.notify(`Model ${config.provider}/${config.model} is configured but unavailable (missing credentials).`, "warning");
                }
            }
        }
        catch {
    ...
    pi.registerCommand("vitals", {
        ...
        handler: async (_args, ctx) => {
            ...
            ctx.ui.notify(`Lineage: ${lineage.mutations} local mutations, ${lineage.divergence} commits behind germline.`, "info");
        },
    });
