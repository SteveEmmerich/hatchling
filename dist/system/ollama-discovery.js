...
When the user confirms they're done, respond with exactly: "DISCOVERY_COMPLETE"`;
    ...
    if (useHindbrain) {
        assistantMessage = await hindbrainPrompt("Begin the discovery conversation. Introduce yourself as a new hatchling.", { systemPrompt });
    }
    else if (ollama) {
    ...
    while (!assistantMessage.includes('DISCOVERY_COMPLETE')) {
        ...
        if (useHindbrain) {
            ...
            assistantMessage = await hindbrainPrompt(`${context}\n\nassistant:`, { maxTokens: 512 });
        }
        else if (ollama) {
            const response = await ollama.chat({
                model: modelId,
