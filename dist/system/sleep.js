...
async function synthesizeExperience(root, day) {
...
export async function sleep() {
    ...
    const snapshot = {
        ...
        mutationState: JSON.parse(await fs.readFile(await PathGuard.validatePath('brain/mutation_state.json', 'read'), 'utf-8')),
        quotas: JSON.parse(await fs.readFile(await PathGuard.validatePath('brain/quotas.json', 'read'), 'utf-8')),
    ...
    const gitAddCode = await new Promise((resolve) => {
    ...
    const gitCommitCode = await new Promise((resolve) => {
